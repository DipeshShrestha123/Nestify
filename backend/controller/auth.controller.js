...
const LogIn = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { message: "User login successful" }));
  } catch (err) {
    console.error(err);
    return res
...
const LogOut = async (req, res) => {
  try {
    ...
    const errorResponse = new ApiResponse(
      err.statusCode || 500,
      null,
      err.message || "User logout failed"
    );
    return res.status(errorResponse.statusCode).json(errorResponse);
  }
};
const checkAuthStatus = (req, res) => {
  ...
  if (!token) {
    return res
      .status(401)
      .json({ isLoggedIn: false, message: "Unauthorized: No token provided" });
  }
...
export { SignUp, LogIn, LogOut, checkAuthStatus, fetchUser };
